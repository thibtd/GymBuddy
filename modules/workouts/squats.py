...
class Squats(Workout):
    ...
    def get_form(self) -> bool:
        ...
        if not self.down:
            ...
            if self.angles['knee'] < up_threshold and self.angles['knee'] > down_threshold:
               print(f'moving state: {self.angles["knee"]< up_threshold and self.angles["knee"] > down_threshold}')
               hips_angles_bool= True
            else:
                hips_angles_bool = (hips_angle < (90+ deviation) and hips_angle > 45-deviation)
        ...
        if not hips_angles_bool:
            if self.down:
                form_issues.append(f"hips angle not between 45 and 90 degrees (angle: {hips_angle:.1f} degrees)")
            else:
                form_issues.append(f"You are not standing straight (angle: {hips_angle:.1f} degrees)")
        ...
        if form_issues:
            ...
            self.fix_form = f"Good form! Keep Going!"
            return True
